...
def get_exchange_rate(currency, date):
    ...
    rate = fx_data.iloc[date_row_index.item() - NUM_METADATA_ROWS, currency_col_index]
...
def generate_fif_report(name, data):
        ...
        for date, transactions in data.groupby(data['Date']):
            ...
            for index, row in transactions.iterrows():
                ...
                fee_str = f'{currency} 0.00' if pd.isna(row['Fees']) else row['Fees'] 
            ...
            day_cost_nzd = (day_cost * (Decimal(1) / exchange_rate)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
            ...
            pdf.set_fill_color(255, 255, 255)
            pdf.set_font('Arial', '', 12)
            pdf.cell(206, 10, f'{currency} {day_cost} × (1 ÷ {exchange_rate}) = ', 1, 0, 'R', 1)
